Feature: Highlight Create validations
   ...
   Scenario Outline: ADT user retrieves event counts
     ...
     Examples:
       ...
       | alok_inst@swlearning.com | course    | course-key     | MTPP4F4PPXJK | reverse | HIGHLIGHT       | CREATE   | 9781133354215 | GTM         |

   @CAP-136
   Scenario Outline: Mindtap users performs a Highlight in Reader
...
#     And I navigate to <Section> section
   ...
   @CAP-136
   Scenario Outline: Mindtap users Verify the Highlight Create event in Analytics Tool
     Given As a user, I launch ADT URL with the following query string <QueryType> , <QueryAttribute> , <Parameter> , <Tab> , <ISBN>
     And I search and select a user with <Search_Data> in SSO UID
     When I search events with the following parameter <EventCategory>,<EventAction>,<EventSource>
     ...
     Examples:
       ...
       | alok_inst@swlearning.com | course    | course-key     | MTPP4F4PPXJK | reverse | HIGHLIGHT       |      CREATE     | 9781133354215 | GTM         | 1     |
