Feature: Search event validations
   @CAP-121
   Scenario Outline: Mindtap users Calculate the Total Search event in Analytics Tool
     ...
     Examples:
       ...
       | alok_inst@swlearning.com | course    | course-key     | MTPPR1GPNWJG | reverse | SEARCH    | LAUNCH      | 9781133354215 | GTM         |

  @CAP-121
   Scenario Outline: Mindtap users performs a Search event
  ...
  @CAP-121
   Scenario Outline: Mindtap users Verify the Search event in Analytics Tool
    Given As a user, I launch ADT URL with the following query string <QueryType> , <QueryAttribute> , <Parameter> , <Tab> , <ISBN>
    And I search and select a user with <Search_Data> in SSO UID
    When I search events with the following parameter <EventCategory>,<EventAction>,<EventSource>
    ...
    Examples:
      ...
      | alok_inst@swlearning.com | course    | course-key     | MTPPR1GPNWJG | reverse | SEARCH    | LAUNCH      | 9781133354215 | GTM         | 1     |
